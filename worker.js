// Backend WebSocket com Durable Objects para Rachacuca Casino v2.5
export class RoomDO {
  constructor(state, env) {
    ...
    this.maxRodadas = 15;
    this.rodadasEliminacao = [];
  }

  // CALCULA RODADAS DE ELIMINAÇÃO BASEADO NO NÚMERO DE JOGADORES
  calcularRodadasEliminacao(totalJogadores) {
    const config = {
      2: { total: 8, eliminacoes: [] },
      3: { total: 10, eliminacoes: [5, 10] },
      4: { total: 15, eliminacoes: [7, 11, 15] },
      5: { total: 18, eliminacoes: [6, 9, 12, 15, 18] },
      6: { total: 20, eliminacoes: [5, 9, 13, 16, 18, 20] },
      7: { total: 22, eliminacoes: [5, 7, 9, 11, 13, 15, 22] },
      8: { total: 25, eliminacoes: [4, 8, 12, 16, 19, 23, 25] }
    };

    const cfg = config[totalJogadores] || config[4];
    this.maxRodadas = cfg.total;
    this.rodadasEliminacao = cfg.eliminacoes;
  }
  ...
  async handleStart(socket) {
    ...
    // CALCULAR RODADAS DE ELIMINAÇÃO
    this.calcularRodadasEliminacao(ativos.length);
  ...
  iniciarNovaRodada() {
    ...
    // VERIFICAR SE É RODADA DE ELIMINAÇÃO
    if (this.rodadasEliminacao.includes(this.numeroRodada) && this.numeroRodada > 1) {
  ...
  finalizarRodada() {
    ...
    // BÔNUS POR ACERTAR O ALVO EXATO
    if (vencedor.distancia === 0) {
      premio += 500;
    }
    ...
    this.broadcast({
      ...
      premio,
      msgExtra: vencedor.distancia === 0 ? '🎯 ALVO EXATO! +500 BÔNUS!' : ''
    });
...
export default {
  async fetch(request, env) {
    ...
    return new Response(`
      🧮 Rachacuca Casino Backend v2.5 FINAL CORRIGIDO
      
      Status: ✅ ONLINE - CERTIFICADO
      ...
      ✅ CORREÇÃO: Rodadas de eliminação funcionando corretamente
    `, {
      headers: { 
